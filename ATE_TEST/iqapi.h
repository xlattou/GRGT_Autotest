...
// Version Information
// ****************************************************************
#define IQAPI_VERSION                        "1.8.77"
#define IQAPI_RELEASE_DATE                   "(Dec 14, 2011)"

#define LPC_VERSION_TO_MATCH                 "1.0.14"
#define FW_VERSION_TO_MATCH                  "0.4.67"
#define FW_GF_INTERFACE_VERSION_TO_MATCH     "0.51.5"
#define FW_GF_VERSION_TO_MATCH               "1.0.2"
...
//! Initializes API libraries  初始化API库
/*!
...
// ****************************************************************
//Specify 详细说明
//! Specifies the analysis parameters for modulation(调制) multi-wave  
...
// ****************************************************************
// Wave Generator Parameters Class   波形发生器参数类
// ****************************************************************
...
public:	
    ...
    int			mcsIndex;	            //!< Sets modulation and coding scheme（编码方案）. Supported range: 0 to 76
...
public:
    ...
    // ****************************************************************
    // Connection Handling Functions   连接处理函数
    // ****************************************************************    
    int ConInitBySerialNumber(char *ip1, char *sn1, char *sn2, char *sn3=NULL, char *sn4=NULL);   //!< For IQ2010 used as nxn system
    int ConInitBySerialNumber(char *ip1, char *sn1, IQV_CONNECTION_TYPE connectionType = IQV_CONNECTION_IQ201X); //!< For IQ201x only. Allow to specify which tester ( if more than one tester connected to the PC) to be connecetd. Note: By default, the tester connected has the port number to be 4000. For connecting to the second tester, the port number has to be different than 4000. Ex., Say 4001, then, ip1 should be specified along with a port number. like, 127.0.0.1:4001
    int ConInit(char *ip1, IQV_CONNECTION_TYPE connectionType = IQV_CONNECTION_IQ201X); //!< Connects to and initializes a single test system.		
    ...
    */ 
    int ConOpen(char *ip1=NULL, char *ip2=NULL, char *ip3=NULL, char *ip4=NULL, IQV_CONNECTION_TYPE connectionType = IQV_CONNECTION_IQ201X);  //!< Connects to or reconnects to the test systems.
    ...
    */
    ...
    // ****************************************************************    
    // Set-up and Configuration Functions   设置配置函数
    // ****************************************************************    
    ...
    */
    ...
    // ****************************************************************    
    // Wave Generation and Downloading Functions 波形产生和下载函数
    // ****************************************************************    
    ...
    */
    ...
    // ****************************************************************    
    // Multi-wave add, download, select Functions  多波增加、下载、选择函数
    // ****************************************************************    
    ...
    // ****************************************************************    
    // Data Capture and Analysis Functions  数据获取和分析函数
    // ****************************************************************    
    int Capture();  //!< Performs a data capture 执行数据捕获
    ...
    */	//使用当前hndl->rx的设置获取数据。如果执行成功，hndl->data会指向一个iqapiCapture对象，这个对象包含样本数据
    int Capture(int vsaNum); //!< Performs a data capture on a single VSA in an IQnxn test system configuration  在IQnxn测试系统配置   简单VSA执行数据获取
    ...
    */
    ...
    // ****************************************************************    
    // Multiport Test Adaptor Functions  多端口测试适配器函数
    // ****************************************************************    
    ...
    */
    ...
    // ****************************************************************    
    // iqapiHndl Member Variables  成员变量
    // ****************************************************************    
...
public:
    ...
    */
    ...
    // ****************************************************************    
    // Internal use functions 内部使用的函数
    // ****************************************************************    
    ...
    // ****************************************************************    
    // Private iqapiHndl Members   私有成员
    // ****************************************************************    
private: 
    bool        connected;
...
#ifdef _INCLUDE_GF_
    int  ConnectGpsFm(char *ip1, char *sn1 = "0");
